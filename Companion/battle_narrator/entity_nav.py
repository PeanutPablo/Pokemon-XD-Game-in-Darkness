...
class EntityNavigator:
    ...
    def __init__(
        ...
        self.on_auto_repeat_change = None
        """Called with the new value when `ctrl+l` flips
        `auto_repeat_enabled`, so the settings store can persist it. None
        in tests and in any build without a settings store; the hotkey
        still works, the choice just does not survive a restart."""
    ...
    def _toggle_auto_repeat(self):
        """Flip the stand-still re-announcement and say what it now is.

        This is what `ctrl+l` is for. It switches off the UNPROMPTED
        repeat -- the one that fires on its own once the player stops
        moving -- and deliberately leaves the manual keys alone: `repeat`
        still works, because turning off an interruption is not the same
        as giving up the ability to ask.

        The confirmation is not optional. The player cannot see a
        checkbox, and the thing being switched is something that happens
        when they do nothing, so without a spoken result the only way to
        learn the state is to stand still and wait to find out.

        `on_auto_repeat_change` is how the settings store hears about it,
        so a toggle made in play is saved and the menu shows the same
        value next time it is opened. Without it the hotkey and the menu
        would be two opinions about one setting, and the menu would win at
        the next launch."""
        self.auto_repeat_enabled = not self.auto_repeat_enabled
    ...
    def poll_once(self, dialogue_active=False):
        ...
        auto_repeat_hotkey = self.hotkeys.get("auto_repeat")
        auto_repeat_fired = (
            auto_repeat_hotkey.poll() if auto_repeat_hotkey else False)
        if not self.context_valid:
            ...
            self._reset_auto_repeat()
            return
        if auto_repeat_fired:
            # First, and on its own branch: it is the only action here that
            # is not "tell me about an entity", and it must work even when
            # nothing is selected -- someone silencing the automatic repeat
            # is quite likely to do it before they start cycling.
            self._toggle_auto_repeat()
