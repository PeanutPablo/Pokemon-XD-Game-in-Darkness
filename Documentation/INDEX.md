...
- **[MASTER.md](MASTER.md)** — **start here if you want the whole picture in one place** (2026-08-17). One entry per feature: what it does for the player, which module owns it, how it is verified, and what is still open, with links into the detailed document for each area. It is a map, not a replacement: `ACCESSIBILITY_COVERAGE_MATRIX.md` stays authoritative for per-screen status and `IMPLEMENTATION_ATTRIBUTION.md` for history.
...
- **[XG_COMPATIBILITY.md](XG_COMPATIBILITY.md)** — **the first real answer to whether any of this works on XG** (2026-08-11), and the **normative data-sourcing rules** that follow from it: one code path serves vanilla and XG, with no game-identity switch, because nothing outside the data distinguishes the two builds. Read the "Data sourcing rules" section before touching any loader — it states the moves-versus-abilities distinction (XG replaces move record *contents* within the same ID space; it changes the abilities table's *shape*) and the general rule both are instances of. Four defects found and fixed, all the same mistake in different structures: a repository constant that was true of one build. Read §8 before quoting any result: everything is static analysis, there has been no live run, and only 8 of the 104 profile addresses were actually checkable.
- **[FIRST_RUN_AND_RUNTIME.md](FIRST_RUN_AND_RUNTIME.md)** — how a player gets from a downloaded zip to a running game (2026-08-18), and why the old flow — install Python 3.12, then type two absolute paths into a console — was the least accessible thing in the project. Covers `setup_discovery.py`'s search and ranking rules, the one ground on which it is allowed to reject an image (disc magic, read from the file; never a filename), and the bundled `Runtime/`: a pinned CPython embeddable package with the required wheels already in it, so setup installs nothing and needs no internet. **Read §5 before adding any import to a release entry point** — the embeddable package has no `venv`, `ensurepip` or `tkinter`, and a module-level `import venv` shipped a release that died before printing a line. §7 lists what is still unverified, including that the live launch path has never been run from a bundled release.
...
14. **[REGION_ROUTING_LIVE_TEST_PLAN.md](REGION_ROUTING_LIVE_TEST_PLAN.md)** — One-action-at-a-time live validation for region-aware routing: what to capture per entity category, what counts as success, and which refusals are expected rather than defects. Not yet run.
15. **[SETTINGS_MENU.md](SETTINGS_MENU.md)** — The companion's own F1 settings menu (2026-08-16): keys and navigation model, what it exposes, why it needs a `WH_KEYBOARD_LL` hook rather than the polled hotkeys everything else uses (F1 is Load State Slot 1, the arrows are the analog stick), how a setting reaches a live reader, and the live test that is still outstanding.
16. **[UNKNOWNS_AND_BLOCKERS.md](UNKNOWNS_AND_BLOCKERS.md)** â€” Everything unresolved, split into: blocked on your XG game files, blocked on a decision only you can make, genuinely open technical questions, and what was deliberately left out of scope.
