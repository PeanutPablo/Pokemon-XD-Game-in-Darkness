...
- Severity: 3
- Story requirement: none
- Technical findings: `entity_nav.py`'s `_refresh()`, `ctrl+shift+slash`
- Implementation status: Implemented
- Live-test status: N/A (mechanical hotkey addition; not separately flagged for dedicated live confirmation beyond the existing entity-nav validation)
- Regression-test status: Regression-tested (`RefreshTests` in `test_entity_nav.py`)
- Last verified date: 2026-07-28
- Remaining work: none
- Notes: distinct from `repeat` — rebuilds `frozen_order`, catching newly-appeared entities
...
- Story requirement: none
- Reproduction steps: select an NPC in entity-nav, press `ctrl+shift+t`
- Relevant save/save-state: none captured
...
- Implementation status: Implemented as `battle_narrator/bag_menu.py` (`HeroItemArraySource`: raw item-record reads with empty-slot skipping; `BagMenuModel`: pure "what's selected" read, no speech, now including the trailing close row via `is_close`; `BagMenuReader`: the speech adapter -- category/item/quantity/description on open or category change, item/quantity/description only on plain cursor movement, "No items." for empty categories, "Close." for the trailing row (combined with "No items." when the category is empty, since row 0 is then both), clean close/reopen, dedup by (category, row)). Top-of-list/bottom-of-list boundary cues were implemented, live-tested, then explicitly removed at the project owner's request (2026-07-29) as unwanted verbosity.
...
- Regression-test status: Regression-tested (`test_bag_menu.py`, 27 tests)
- Last verified date: 2026-07-29
...
- Accessibility status: **Implemented, Regression-tested, NOT yet live-tested** (2026-08-09) — a `bridge` entity-navigation category publishes the connections the current alignment offers, updated live from flag 968, plus the pre-existing passive announcer
- Current limitation: **connection points only** — bridge *controls* are still not entities and `_RAW_PAD_TRANSITIONS`' 16 coordinate boxes remain in `gateon_bridge.py` (unused by the new category); the live `GScolsys2` enable state is still never read, so routing is not alignment-aware and the **plain beacon (ctrl+shift+g) should be used for bridge connections rather than the routed guide**; no alignment transition has ever been observed live (all 17 `GATEON BRIDGE` log lines report alignment 0)
- Severity: 1
- Story requirement: reached
- Technical findings: general flag 968 drives `M6_out`'s `pier_def`, which toggles CCD entries 23-31 — the bridge **segments** (no walk model of their own). **`enable == 1` means that direction is CONNECTED**, settled 12/12 vs 0/12 against two independent sources (the script's own table and the `ALIGNMENTS` prose written from the real game); `test_bridge_connections.PolarityTests` pins both halves. The state table is parsed from the extracted room script and the decks (walk entries 58/59), their "northern"/"southern" naming, the compass direction of each segment and every position are derived from `M6_out.ccd` — no Gateon coordinate is written into the companion. `M6_out` also holds five room-script interaction records (`pier_trouble` ×2, `door_hoseb`, `ev_mechakyogre_check`, `crabcrab_in_col`); `pier_trouble` is the lead for the control pads. Full record: [GATEON_BRIDGE_ACCESSIBILITY.md](GATEON_BRIDGE_ACCESSIBILITY.md)
- Regression-test status: Regression-tested (`test_bridge_connections.py`, 29 tests, including 12 real-`M6_out` cases)
- Remaining work: live-validate the connection points (walk to one, then change alignment and confirm the list changes); then the `GScolsys2` live read, what a present segment does to walkability, alignment-aware routing, and bridge controls
...
- Implementation status: Not implemented
- Remaining work: confirm the full cursor-value-to-field-position mapping (one more live session), then read whichever of the already-working name-resolution paths applies per position (the `vs_top_target_status_window_id`/`vs_bottom_target_status_window_id` status windows already correctly show target names, or the `FightSide`/`FightTrainer` struct chain from the remaining-count entry below), and add menu ID 92 to `menus.py`'s recognized set
...
- Implementation status: Not implemented — added temporary diagnostic logging (`narrator.py`'s `_debug_dump_trainer_parties`) alongside the send-out investigation above, to passively capture a fresh battle's data (ideally one where the opponent hasn't used every Pokemon yet) from the project owner's next battle
- Remaining work: resolve the not-yet-sent-out-vs-fainted ambiguity from fresh log data, then implement as a new on-demand hotkey (mirroring `BattleHPSummary`'s existing hotkey-triggered-summary pattern, per the project owner's earlier explicit preference for that pattern over passive narration for this kind of on-demand check)
...
- Implementation status: **Heart Gauge Implemented (2026-07-30)**, per the project owner's explicit "both" answer to how it should be exposed: (1) `party.py`'s `_dark_status()` computes `heart_gauge_percent` (`round_percent(max_point - current, max_point)`) alongside the existing Shadow-move override, exposed on `PartySlot`; `party_summary_screen.py`'s Status page speaks "Heart Gauge: fully open, ready to purify." or "Heart Gauge: N percent open." whenever present. (2) A new dedicated `HeartGaugeSummary` hotkey class (`hotkeys.py`, default `ctrl+shift+j`) speaks the same for every currently-Shadow party member on demand, usable anywhere in the overworld — modeled on `BattleHPSummary` but without its two-phase settling logic (not needed outside battle's animating HP bars). Wired through `profile.py`/`phase1b_app.py`/`phase1b_lifecycle.py` following the project's standard hotkey-reader pattern.
...
- Primary workstream: Infrastructure, safety, testing, and distribution
- Discovery status: Discovered
- Accessibility status: Investigating (research-only)
- Current limitation: `DISTRIBUTION_PIPELINE.md` exists as research; nothing packaged
- Remaining work: not currently prioritized (see priority order in the master plan — distribution is last)

### User-facing documentation
- Primary workstream: Infrastructure, safety, testing, and distribution
- Discovery status: Discovered
- Accessibility status: Unknown
- Current limitation: `Start Battle Narrator.bat` exists for manual launching; no end-user guide (hotkey list, what each feature does) exists yet for the project owner's own independent reference
- Remaining work: not currently prioritized
...
- Coupon shop item and quantity screens: read the item's Coupon Price (+0x08)
  ...
  Pokédollars.
- `Ctrl+Shift+M`: always announces Pokédollars and Poké Coupons separately,
